...
class TabularDataToolkit(AsyncBaseToolkit):
    ...
    def _load_tabular_data(self, file_path: str) -> pd.DataFrame:
        ...
        if file_ext == ".csv":
            ...
            df = self._load_excel_with_merged_cells(file_path)
        elif file_ext == ".json":
    ...
    def _load_excel_with_merged_cells(self, file_path: str) -> pd.DataFrame:
        """Load Excel file with merged cells support.

        This method uses openpyxl directly to handle merged cells properly,
        which is not supported in pandas read_excel with read_only=True mode.

        Args:
            file_path (str): Path to the Excel file.

        Returns:
            pd.DataFrame: DataFrame containing the Excel data with merged cells expanded.
        """
        try:
            # Load workbook in non-read-only mode to access merged cells
            wb = openpyxl.load_workbook(file_path, read_only=False, data_only=True)

            # Get the first sheet (or you can specify a sheet name)
            sheet_names = wb.sheetnames
            if not sheet_names:
                raise Exception("Excel file contains no worksheets")
            ...
            for merged_range in sheet.merged_cells.ranges:
                ...
                # Store the value for all cells in the merged range
                for row in range(merged_range.min_row, merged_range.max_row + 1):
                    for col in range(merged_range.min_col, merged_range.max_col + 1):
                        merged_values[(row, col)] = value

            # Read data row by row, using merged values where applicable
            data = []
            for row_idx, row in enumerate(sheet.iter_rows(values_only=False), 1):
                row_data = []
                for col_idx, cell in enumerate(row, 1):
                    # Use merged value if available, otherwise use cell value
            ...
            if len(df) > 0:
                # Check if first row contains mostly strings (potential headers)
                first_row = df.iloc[0]
                non_null_count = first_row.notna().sum()
                if non_null_count > 0:
                    string_count = sum(1 for val in first_row.dropna() if isinstance(val, str))
                    if string_count / non_null_count > 0.5:  # More than 50% strings
            ...
            logger.warning(f"Failed to load Excel with merged cells support: {e}. Falling back to pandas.")
            # Fallback to regular pandas read_excel if openpyxl method fails
